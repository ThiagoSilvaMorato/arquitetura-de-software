...
export class DoctorWorkingHoursController {
  ...
  async addWorkingHours(req, res) {
    try {
      ...
      const doctor = await this.workingHoursService.addWorkingHours(id, day, timeSlot);
  ...
  async removeWorkingHours(req, res) {
    try {
      const { id } = req.params;
      const { day, timeSlot } = req.body;
      const doctor = await this.workingHoursService.removeWorkingHours(id, day, timeSlot);
      res.status(200).json(doctor);
    } catch (error) {
      if (error.message === "Doctor not found.") {
        res.status(404).json({ error: error.message });
